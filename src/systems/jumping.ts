...
const jumpableComponents: [typeof Transform, typeof Body, typeof Jump] = [Transform, Body, Jump];
...
export class JumpingSystem {
    ...
    run(dt: number) {
        ...
        for (let jumpable of jumpables) {
            const [transform, body, jump] = this.storage.getComponents(jumpable, jumpableComponents);
